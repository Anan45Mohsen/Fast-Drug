...
router.put("/register/:id", auth,checkRole(USER_ROLE , ASSOCIATION_ROLE),async (req, res) => {
  await registerController.updateUserInfoById(req, res)
})

router.delete("/register/:userId", auth,checkRole(USER_ROLE , ASSOCIATION_ROLE),(req, res) => {
